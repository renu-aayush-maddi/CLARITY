# # backend/app/utils/ingest_excel.py
# import pandas as pd
# import numpy as np
# import re
# import logging
...
def ensure_subjects_exist(db: Session, df: pd.DataFrame, study_name: str):
    if 'subject_id' not in df.columns: return

    # Keep Site ID if exists
    cols_to_keep = ['subject_id']
    if 'site_id' in df.columns: cols_to_keep.append('site_id')
    ...
    for _, row in subjects.iterrows():
        unique_id = str(row['subject_id']).strip() 
        site_val = str(row['site_id']) if 'site_id' in subjects.columns and pd.notna(row['site_id']) else "Unknown Site"
...
def ingest_file(file, db: Session, study_name: str = None):
    ...
    if not study_name:
         return {"status": "error", "reason": "Study Name is missing. Please select a Study."}
    ...
    try:
        ...
        for sheet_name, df_raw in dfs.items():
            ...
            # 1. Smart Header Detection (Handle Multi-Row Headers)
            header_idx = find_header_row(df_raw)
            
            # CHECK FOR SPLIT HEADERS (The CPID Fix)
            # If the next row also looks like it has data, merge them
            try:
                row0 = df_raw.iloc[header_idx].astype(str).replace('nan', '').replace('NaN', '')
                row1 = df_raw.iloc[header_idx + 1].astype(str).replace('nan', '').replace('NaN', '')
                
                # Check if Row 1 has "Missing Visits" or similar metrics
                row1_text = " ".join(row1).lower()
                if "missing" in row1_text or "queries" in row1_text:
                    # COALESCE HEADERS: Use Row 1 (Metric) if present, else Row 0 (ID)
                    new_header = []
                    for h0, h1 in zip(row0, row1):
                        clean_h1 = h1.strip()
                        new_header.append(clean_h1 if clean_h1 else h0.strip())
                    
                    df_content = df_raw[header_idx + 2:].copy()
                    df_content.columns = new_header
                else:
                    # Standard Single Row Header
                    df_content = df_raw[header_idx + 1:].copy()
                    df_content.columns = df_raw.iloc[header_idx]
            except:
            ...
            for col in ['days_missing', 'days_outstanding', 'missing_pages', 'missing_visits', 'open_queries', 'protocol_deviations']:
                if col in df_clean.columns:
                    df_clean[col] = pd.to_numeric(df_clean[col], errors='coerce').fillna(0)

            # 5. Create Subjects & Insert
            ensure_subjects_exist(db, df_clean, study_name)
